...
class AttributeRepository implements AttributeRepositoryInterface {
    ...
    public function getAll(): array {
        return $this->db->query("SELECT * FROM attributes ORDER BY name ASC")->fetchAll(\PDO::FETCH_CLASS, Attribute::class, [$this->logger]);
    }

    public function findById(int $id): ?Attribute {
        $stmt = $this->db->prepare("SELECT * FROM attributes WHERE id = ?");
        $stmt->setFetchMode(\PDO::FETCH_CLASS, Attribute::class, [$this->logger]);
        $stmt->execute([$id]);
    ...
    public function getValues(int $attributeId): array {
        ...
        return $stmt->fetchAll(\PDO::FETCH_CLASS, AttributeValue::class, [$this->logger]);
    }
